...
const getThreadById = async (req, res) => {
  try {
    ...
    if (!threads) {
      return res
        .status(404)
        .json({ message: "Chưa có cuộc trò chuyện, hãy bắt đầu với ai đó !" });
    }

    const threadsFormatted = await Promise.all(
      threads.map(async (thread) => {
        const user1 = await User.findById(thread.user1Id);
        const user2 = await User.findById(thread.user2Id);

        const messages = await Promise.all(
          thread.msgIds.map(async (msgId) => {
...
const startMessage = async (req, res) => {
  try {
    ...
    if (!seller)
      return res
        .status(404)
        .json({ message: "Người dùng đã bị cấm hoặc xóa tài khoản !" });

    if (sellerId === userId) {
      return res
        .status(401)
        .json({ message: "Không thể chat với chính bản thân mình!" });
    }
