...
module.exports.getCarBySlug = async (req, res) => {
  try {
    ...
    const resultFindingCar = await Car.findOne(find).select(
      "-__v -deleted -_id"
    );
...
module.exports.getCarsDisplay = async (req, res) => {
  try {
    ...
    if (user.role !== "seller")
      return res.status(401).json({ message: "Phải đăng ký làm người bán" });
  ...
  try {
    ...
    if (status) {
      if (car.status === "sold") {
        return res
          .status(403)
          .json({ message: "Xe đã bán không thể sửa được !" });
      }
