...
function RequestDetail(){
    ...
    useEffect(() => {
        const getRequest = async () => {
            try {
                const responseRequest = await axios.get(`${import.meta.env.VITE_SERVER_URL}/api/v1/requestAdd/${slugRequest }`, {
    ...
    const getNavLinkClass = ({ isActive }) => {
        return isActive ? "requestDetail__progress__current" : "requestDetail__progress__future";
    }
            ...
            <div className="requestDetail">
                ...
                <div className="requestDetail__progress__contain">
                    <div className="requestDetail__progress">
                        <NavLink to={`/request-detail/${slugRequest}/info`} className={getNavLinkClass}>
                            1. Điền thông tin
                        </NavLink>
                        <div className="requestDetail__progress__horizontal"></div>
                        <NavLink to={`/request-detail/${slugRequest}/verify`} className={getNavLinkClass}>
                            2. Đợi xác thực
                        </NavLink>
                        <div className="requestDetail__progress__horizontal"></div>
                        <NavLink to={`/request-detail/${slugRequest}/done`} className={getNavLinkClass}>
                            3. Xác thực
                        </NavLink>
                    </div>
                </div>
                ...
                <div className="requestDetail__footer">
                    <div className="requestDetail__buttons">
                        <button onClick={() => {navigate(getLinkPrev())}} disabled={DisabledPrev()} className="requestDetail__navigate__prev">Trang trước</button>
                        <button onClick={() => {navigate(getLinkNext())}} disabled={DisabledNext()} className="requestDetail__navigate__next">Trang sau</button>
                    </div>
                </div>
            </div>
        </>
