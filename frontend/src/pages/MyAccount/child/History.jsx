...
function History() {
  ...
  useEffect(() => {
    const getRequests = async () => {
      try {
        const respondsRequest = await apiClient.get("/requestAdd");
  ...
  const FilterBySearch = (search) => {
    ...
    resultFilter.current = resultFilter.current.filter((request) =>
      regex.test(request.name)
    );
  };
      ...
      <div className="history">
        ...
        <div className="history__header">
          <div className="history__header__title">
            ...
            <span className="history__header__title__span">
              Inspection Cars
            </span>
          </div>
        </div>
        ...
        <div className="history__contain">
          <div className="history__options">
            <ul className="history__status__list">
              ...
              <li
                className={`history__status ${
                  filterRequest.status === "pending" ? "picked" : ""
                }`}
              ...
              <li
                className={`history__status ${
                  filterRequest.status === "checked" ? "picked" : ""
                }`}
                onClick={() => handleChangeStatus("checked")}
              >
              ...
              <li
                className={`history__status ${
                  filterRequest.status === "done" ? "picked" : ""
                }`}
              ...
              <li
                className={`history__status ${
                  filterRequest.status === "reject" ? "picked" : ""
                }`}
            ...
            <div className="history__filters">
              ...
              <div className="history__sort">
                <button
                  className="history__sort__button"
                ...
                currentRequests.map((request, r_index) => (
                  <tr key={r_index} className="history__table__tr">
                    ...
                    <td className="history__table__td--title">
                      ...
                      <span className="history__table__item__name">
                        {shortenString(request.name)}
                      </span>
                    </td>
                    ...
                    <td className="history__table__td--status">
                      <span data-type={request.status}>
                        {converStatusRequest(request.status)}
                      </span>
                    </td>
                    <td className="history__table__td--functions">
                      <Link
                        to={`/request-detail/${request.slug}/${
                      ...
                      >
                        ...
                        <span className="history__functions__span">
                          Chỉnh sửa
                        </span>
                      </Link>
                      {request.status === "pending" && (
